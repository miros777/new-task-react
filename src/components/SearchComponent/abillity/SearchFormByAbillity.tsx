...
const SearchFormByAbillity = () => {
    ...
    const customHandler = async (formData: FormProps) => {
        try {
            let pokemon = await pokemonApiSearch.getAllByAbility(formData.searchword)
                .then(v => v);
            navigate('search_abillity', {state: {pokemon: pokemon}});
        }catch (e){
            const error = e as AxiosError;
            // console.log(error.message);
            navigate('search_abillity/error');
        }
    }
    return (
        <>
            <form onSubmit={handleSubmit(customHandler)} className="d-flex">
                <label className="d-flex f-column">
                    <input placeholder="search any abillity..." type="text"  {...register('searchword')}/>
...
export default SearchFormByAbillity;